...
export default async (e: any, next: () => void) => {
    ...
    try {
        if (cmd.startsWith('安装依赖')) {
            ...
            await manager.start();
            message.send({ format: Format.create().addText('Yunzai Worker 已启动') });
            ...
            // 更新后重新安装依赖（可能有新插件/新依赖）
            await manager.installDeps();
            const result = Format.create().addText(`更新完成\n${out.slice(0, 200)}`);
            message.send({ format: result });
            if (wasRunning) {
                await manager.start();
                message.send({ format: Format.create().addText('Yunzai Worker 已自动重启') });
            }

        } else if (cmd.startsWith('启动')) {
            fmt.addText('正在启动 Yunzai Worker...');
            message.send({ format: fmt });
            await manager.start();
            message.send({ format: Format.create().addText('Yunzai Worker 已启动') });
            ...
            await manager.stop();
            fmt.addText('Yunzai Worker 已停止');
            message.send({ format: fmt });

        } else if (cmd.startsWith('重启')) {
            fmt.addText('正在重启 Yunzai Worker...');
            message.send({ format: fmt });
            await manager.restart();
            message.send({ format: Format.create().addText('Yunzai Worker 已重启') });
